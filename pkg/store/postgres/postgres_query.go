...
func agentMatchesQuery(t *agent.Agent, query *agent.Query) bool {
	if t == nil || query == nil {
		return true
	}
	...
	if query.NamespaceID != "" && t.NamespaceID != query.NamespaceID {
		return false
	}
	if len(query.Tags) > 0 {
	...
	if query.Metadata != nil {
		for key, want := range query.Metadata {
			...
			if key == queueMetadataKey {
				if t.Metadata == nil || t.Metadata[key] != want {
					return false
				}
				continue
			}
